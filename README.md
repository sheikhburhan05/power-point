...
### Structured content JSON (no LLM, no API key)

Use your own pre-written content in JSON format — **nothing is generated by Claude**:
  ...
  "content_sections": [
    {
      ...
      "bullets": [
        { "header": "Product design and localization", "content": "Develops irrigation hardware..." }
      ]
    }
  ]
}
...
python main.py run \
  --input input/slide.pptx \
  --structured-content source.md \
  --out-dir output/
```

The mapper assigns, in order:
- `title` → main title block
- `subtitle` → section headings (e.g. "Primary Activities")
- `header` → short label blocks
- `body` → long paragraph blocks

Example file: `examples/irrigation_value_chain.json` or your `source.md` (valid JSON).

**Priority:** `--content-json` > `--structured-content` > Claude (`--topic` / `--source-text`)
...
- **Mixed inline formatting**: Multi-run styles within one block are simplified to the dominant run style; per-run colors are preserved when the original shape had multiple runs.
- **Font color**: RGB and theme (scheme) colors are extracted into `style_hint` and reapplied. Inherited/default colors are mapped to theme `TEXT_1` (typically dark body text).
